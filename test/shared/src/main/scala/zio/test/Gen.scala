...
final case class Gen[-R, +A](sample: ZStream[R, Nothing, Sample[R, A]]) { self =>
   ...
   * }}}
   */
  def filter(f: A => Boolean)(implicit trace: Trace, sized: Sized): Gen[R, A] =
   ...
   */
  def filterNot(f: A => Boolean)(implicit trace: Trace, sized: Sized): Gen[R, A] =
    filter(a => !f(a))

  def withFilter(f: A => Boolean)(implicit trace: Trace, sized: Sized): Gen[R, A] = filter(f)
   ...
   */
  def noShrink(implicit trace: Trace, sized: Sized): Gen[R, A] =
   ...
   */
  def resize(size: Int)(implicit trace: Trace, sized: Sized): Gen[R, A] =
...
object Gen extends GenZIO with FunctionVariants with TimeVariants {
   ...
   */
  def bounded[R, A](min: Int, max: Int)(f: Int => Gen[R, A])(implicit trace: Trace, sized: Sized): Gen[R, A] =
   ...
   */
  def concatAll[R, A](gens: => Iterable[Gen[R, A]])(implicit trace: Trace, sized: Sized): Gen[R, A] =
   ...
   */
  def constSample[R, A](sample: => Sample[R, A])(implicit trace: Trace, sized: Sized): Gen[R, A] =
  ...
  )(implicit trace: Trace, sized: Sized): Gen[R, A] =
    Gen(ZStream.fromIterable(as).take(sized.size).map(a => Sample.unfold(a)(a => (a, shrinker(a)))))
   ...
   */
  def fromZIO[R, A](effect: URIO[R, A])(implicit trace: Trace, sized: Sized): Gen[R, A] =
    fromZIOSample(effect.map(Sample.noShrink))

  /**
   * Constructs a generator from an effect that constructs a sample.
   */
  def fromZIOSample[R, A](effect: ZIO[R, Nothing, Sample[R, A]])(implicit trace: Trace, sized: Sized): Gen[R, A] =
  ...
  def oneOf[R, A](as: Gen[R, A]*)(implicit trace: Trace, sized: Sized): Gen[R, A] =
   ...
   */
  def sized[R, A](f: Int => Gen[R, A])(implicit trace: Trace, sized: Sized): Gen[R, A] =
    size.flatMap(f)
   ...
   */
  def suspend[R, A](gen: => Gen[R, A])(implicit trace: Trace, sized: Sized): Gen[R, A] =
   ...
   * }}}
   */
  def weighted[R, A](gs: (Gen[R, A], Double)*)(implicit trace: Trace, sized: Sized): Gen[R, A] = {
